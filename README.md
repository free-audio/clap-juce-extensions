...
3. It presents a set of completely optional extensions which break JUCE abstractions to allow extended CLAP feature
   support and
4. We do not support JUCE-based CLAP hosting
...
Given a stating point of a JUCE plugin using CMake which can build a VST3, AU, Standalone and so forth with
...
Most importantly, in the three critical places a DAW interacts with a plugin - CLAP id, param IDs, and state streaming -
...
While there is no guarantee an official JUCE implementation, if it were to exist, would make these choices,
it seems quite natural that they would, and in that case, your plugin would continue to work.
...
2. In C++, the `wrapperType` API doesn't support CLAP. All CLAP plugins will define a `wrapperType` of
   `JuceWrapperType_Undefined`. We do provide a workaround for using our extensions mechanism, below.
...
Surge is a worked example of using many of these. We are also happy to discuss them - reach out in the issues here
or in a shared discord server.
...
* The CMake API is not consistent. Rather than add "CLAP" as a plugin type, you need a couple of extra lines of CMake to
  activate your CLAP
* We cannot support the `wrapperType` api, as discussed above
